...
document.querySelectorAll(".charKey").forEach(function (charKeyBtn) {
  charKeyBtn.addEventListener("click", function () {
    const value = charKeyBtn.dataset.value; // dataset é usado para acessar as tags datas do HTML, ou seja, ele já começa com um valor pré-definido
    input.value += value;
  });
});
...
document.getElementById('copyToClipboard').addEventListener('click', function(ev){
    ...
    if (button.innerText === 'Copy'){
        ...
        navigator.clipboard.writeText(resultInput.value) //window.navigator
    } else {
